 ...
 void main()
{
            ...
            gl_FragColor =  vec4(texture2D(myTexture1, textureCoordinate));

    
//    if (varyOtherPostion.x > leftBottom.x && varyOtherPostion.x < rightTop.x && varyOtherPostion.y < rightTop.y && varyOtherPostion.y > leftBottom.y) {
...
//        lowp vec4 textColor = texture2D(myTexture1, test);
////        gl_FragColor = vec4(textColor.rgb+backColor.rgb*(1.0-textColor.a),backColor.a);
//
//        gl_FragColor =  vec4(texture2D(myTexture1, textureCoordinate));
//
//    }
//    else {
//
//        gl_FragColor = vec4(texture2D(inputImageTexture, textureCoordinate).rgb, texture2D(inputImageTexture, +textureCoordinate+vec2(-0.5, 0)).r);
//
//    }
//
...
@implementation FUOpenGLView
{
    ...
    GLuint leftBottomUniform;
    GLuint rightTopUniform;
...
- (void)displayPixelBuffer:(CVPixelBufferRef)pixelBuffer withLandmarks:(float *)landmarks count:(int)count MAX:(BOOL)max
{
    ...
    dispatch_async(_contextQueue, ^{  //dispatch_sync  iphone8p 可能死锁
        ...
        if (type == kCVPixelFormatType_32BGRA)
        {
            ...
            [self setupTexture:[UIImage imageNamed:@"aliclient86kpWZzMtHYnPECjpYRI.png"]];
...
//    static CGFloat pointX = 120;
//    static CGFloat pointY = 667;
//    pointX += 10;
//    if (pointX >= 750) {
//        pointX = 120;
//    }
...
//    CGFloat rY = 120;
//    //
//    CGFloat left = (pointX-r-375)/375.0;
//    CGFloat bottom = (pointY-rY-667)/667.0;
//
//    CGFloat right = (pointX+r-375)/375.0;
//    CGFloat top = (pointY+rY-667)/667.0;
    
    glUniform2f(leftBottomUniform, 0, 0);
    glUniform2f(rightTopUniform, 0.5, 0.5);
...
- (BOOL)loadShadersRGBA
{
    ...
    texture1Uniform = glGetUniformLocation(rgbaProgram, "myTexture1");
    
    leftBottomUniform = glGetUniformLocation(rgbaProgram, "leftBottom");
    rightTopUniform = glGetUniformLocation(rgbaProgram, "rightTop");
    
//    glBindAttribLocation(rgbaProgram, leftBottomUniform, "leftBottom");
//    glBindAttribLocation(rgbaProgram, rightTopUniform, "rightTop");
...
- (void)setupTexture:(UIImage *)image{
    ...
    glDeleteTextures(5, &texture1Uniform);
    // 1获取图片的CGImageRef
...
//        CGContextScaleCTM (spriteContext, 1.0, -1.0);
    ...
    glGenTextures(0, &texture1Uniform);
    glBindTexture(GL_TEXTURE_2D, texture1Uniform);
